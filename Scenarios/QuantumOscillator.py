...
def quantum_oscillator():
    ...
    momentum = np.sqrt(2 * m * energy_state)
    print("Momentum (p): ", momentum)

    # Define the momentum symbol
    p = sp.symbols('p')

    # Define the probability density function for momentum
    pdf_momentum = integrand(p)

    # Calculate the RMS of momentum
    rms_momentum = sp.sqrt(quad(lambda p_values: pdf_momentum.subs(p, p_values) * p_values**2, -np.inf, np.inf)[0])
    print("Momentum RMS: ", rms_momentum)
